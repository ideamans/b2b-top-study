---
title: サポート体制
description: 'サービスのサポート体制やカスタマーサクセスの取り組みを紹介するセクションです。「充実のサポート体制」「導入後のサポート」といった見出しのもと、導入前から運用後まで安心して利用できる体制が整っていることを訴求します。'
---

# サポート体制

## 定義

サービスのサポート体制やカスタマーサクセスの取り組みを紹介するセクションです。「充実のサポート体制」「導入後のサポート」といった見出しのもと、導入前から運用後まで安心して利用できる体制が整っていることを訴求します。

## 採用率

**37.1%**（62サイト中23サイト）

約3分の1強のサイトが独立したサポート体制セクションを持っています。

## 典型的な配置位置

中央値は **0.71**（ページ後半71%あたり）です。導入事例の後、CTAの前に配置されるケースが多く、「興味→事例で確信→サポートで安心→CTA」というコンバージョンフローを構成します。

## 位置分布

![分布](/screenshots/histograms/support.png)

## 実例ギャラリー

調査サイトから一例をご紹介します。

#### backlog.com
[backlog.com](https://backlog.com/ja)

![backlog.comのサポート](/screenshots/backlog.com/section-08.jpg)

#### バクラク
[bakuraku.jp](https://bakuraku.jp)

![バクラクのサポート](/screenshots/bakuraku.jp/section-07.jpg)

#### Money Forward クラウド
[biz.moneyforward.com](https://biz.moneyforward.com)

![Money Forward クラウドのサポート](/screenshots/biz.moneyforward.com/section-11.jpg)

#### jinjer
[hcm-jinjer.com](https://hcm-jinjer.com)

![jinjerのサポート](/screenshots/hcm-jinjer.com/section-08.jpg)

#### ジョブカン勤怠管理
[jobcan.ne.jp](https://jobcan.ne.jp)

![ジョブカン勤怠管理のサポート](/screenshots/jobcan.ne.jp/section-13.jpg)

#### KARTE
[karte.io](https://karte.io)

![KARTEのサポート](/screenshots/karte.io/section-07.jpg)

#### Kintone
[kintone.com](https://kintone.com)

![Kintoneのサポート](/screenshots/kintone.com/section-07.jpg)

#### スマレジ
[smaregi.jp](https://smaregi.jp)

![スマレジのサポート](/screenshots/smaregi.jp/section-11.jpg)

#### SmartHR
[smarthr.jp](https://smarthr.jp)

![SmartHRのサポート](/screenshots/smarthr.jp/section-13.jpg)

#### CollaboFlow
[www.collabo-style.co.jp](https://www.collabo-style.co.jp)

![CollaboFlowのサポート](/screenshots/www.collabo-style.co.jp/section-07.jpg)

#### eセールスマネージャー
[www.e-sales.jp](https://www.e-sales.jp)

![eセールスマネージャーのサポート](/screenshots/www.e-sales.jp/section-06.jpg)

#### GMOサイン
[www.gmosign.com](https://www.gmosign.com)

![GMOサインのサポート](/screenshots/www.gmosign.com/section-06.jpg)

#### カオナビ
[www.kaonavi.jp](https://www.kaonavi.jp)

![カオナビのサポート](/screenshots/www.kaonavi.jp/section-10.jpg)

#### Talent Palette
[www.talent-palette.com](https://www.talent-palette.com)

![Talent Paletteのサポート](/screenshots/www.talent-palette.com/section-11.jpg)

#### www.works-hi.co.jp
[www.works-hi.co.jp](https://www.works-hi.co.jp/products/hr)

![www.works-hi.co.jpのサポート](/screenshots/www.works-hi.co.jp/section-05.jpg)

## 見出しの傾向

- **「充実のサポート」型**: 「充実のサポート体制」「充実のサポートメニュー」が最多です。
- **「安心」型**: 「安心のサポート体制」「安心して活用するために」のように安心感を強調します。
- **「伴走」型**: 「伴走サポートでノウハウ面も支援します」のようにカスタマーサクセスを意識した表現です。
- **差別化型**: 「サポート体制でも選ばれる理由があります」のように、サポートを差別化要因として訴求するパターンもあります。

## 設計のポイント

サポート体制セクションは、導入の意思決定における「最後の不安解消」の役割を担います。導入前→導入時→運用後の3フェーズで説明するパターンが多く、ライフサイクル全体をカバーする姿勢を示すことで、訪問者の安心感を高めます。

電話・メール・チャット等のサポートチャネル、専任担当者の有無、レスポンスタイムなど、具体的な情報を明記することが重要です。「導入→運用まで充実サポート」のように導入の流れセクションとの連携も効果的で、CTAの直前に配置することでコンバージョン率の向上に寄与します。

## テキスト事例集

### 見出しの事例一覧

| サイト名 | 見出しテキスト |
|----------|--------------|
| SmartHR | 充実したサポート |
| jinjer | 充実のサポート体制 |
| Helpfeel | 導入時にサポートは受けられるの？ |
| ferret One | 伴走サポートでノウハウ面も支援します |
| TOKIUM | サポート体制でも選ばれる理由があります |
| カオナビ | カオナビだからできる「使いこなせる」充実のサポート |
| COMPANY | 充実のサポートメニュー |

### サポート項目の具体例

| サイト名 | サポート項目 |
|----------|------------|
| SmartHR | eラーニング / 活用セミナー / チャットサポート / 他計8種 |
| jinjer | 初期設定支援 / サポート担当に相談 / 電話相談対応 |

### 企画のヒント

- Helpfeelの「導入時にサポートは受けられるの？」のように、FAQ形式で不安を直接受け止める見出しも効果的です
- 導入前→導入中→運用中の3段階でサポート内容を整理するフォーマットが多く見られます
